...
from lib.feature_extractor.pos_context_sequence_feature_extractor \
    import POSContextSequenceFeatureExtractor
from lib.feature_extractor.length_feature_extractor \
    import LengthFeatureExtractor
from lib.feature_extractor.lexical_sequence_feature_extractor \
    import LexicalSequenceFeatureExtractor
...
class FeatureExtractionFlowMapper(BaseFlowComponent):
    def __init__(self, context_window_size=4, prime_feature_length=4,
        ...
        pos_context_seq_feature_extractor = POSContextSequenceFeatureExtractor(
            k_param=context_window_size,
            prime_feature_length=prime_feature_length,
            add_prime_feature=add_prime_feature_val)
        ...
        lexical_feature_extractor = \
            LexicalSequenceFeatureExtractor(slice_window_length)
        # register the feature extractor to this flow component
        self.register_feature_extractor(lexical_feature_extractor)


        # instantiate any other feature extractor and register it with flow
        # components to start using it in the flow
